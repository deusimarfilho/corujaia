...
function handleDefaultStreamResponseV2(response, stream, responseProps) {
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      // LM Studio (e outros) podem encerrar o stream sem enviar finish_reason.
      // Sem isso a UI fica "travada" mostrando tokens parciais e nunca finaliza.
      if (!settled) {
        if (fullText.length > 0) {
          console.log(
            `\x1b[43m\x1b[34m[STREAMING]\x1b[0m Stream ended without finish_reason; finalizing ${fullText.length} chars.`
          );
        }
        settle(fullText);
      }
      ...
      // Com texto parcial, entrega o que foi gerado em vez de abortar vazio.
      if (fullText.length > 0) {
        console.log(
          `\x1b[43m\x1b[34m[STREAMING]\x1b[0m Returning ${fullText.length} partial chars after stream error.`
        );
        settle(fullText);
      } else {
        settle(fullText, { error: e.message });
      }
    }
  });
}
