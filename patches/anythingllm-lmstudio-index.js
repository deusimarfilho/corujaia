...
class LMStudioLLM {
  ...
  /** Espaço generoso para respostas longas (cruzamentos) dentro da janela de contexto. */
  maxOutputTokens() {
    const configured = Number(process.env.LMSTUDIO_MAX_OUTPUT_TOKENS || 16384);
    const window = this.promptWindowLimit();
    if (!Number.isFinite(configured) || configured <= 0) {
      return Math.max(4096, Math.floor(window * 0.5));
    }
    return Math.min(configured, Math.max(4096, Math.floor(window * 0.55)));
  }
