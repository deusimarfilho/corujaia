...
function findPeopleCrossRefSource(sources = []) {
  return sources.find((source) =>
    String(source?.title || "").includes("[CRUZAMENTO] Pessoas encontradas")
  );
}

function extractPeopleLinesFromCrossRef(text = "") {
  const match = String(text).match(/PESSOAS:\s*\n([\s\S]*)/i);
  if (!match) return [];
  return match[1]
    .split("\n")
    .map((line) => line.trim())
...
function extractFocusFromCrossRef(text = "") {
  const match = String(text).match(/mencionam\s+([^.]+)\./i);
  return match?.[1]?.trim() || "a localidade ou termos da pergunta";
}

function injectStructuredPeoplePrompt(userPrompt = "", sources = []) {
  const crossRef = findPeopleCrossRefSource(sources);
  if (!crossRef?.text) return userPrompt;

  const lines = extractPeopleLinesFromCrossRef(crossRef.text);
  if (lines.length === 0) return userPrompt;

  const focus = extractFocusFromCrossRef(crossRef.text);
  return (
    `${userPrompt}\n\n` +
    `LISTA OBRIGATORIA (${lines.length} pessoas em documentos que mencionam ${focus}):\n` +
    `${lines.slice(0, 35).join("\n")}\n\n` +
    `Responda listando todas as pessoas acima com os arquivos de origem. ` +
    `Nao diga que nao ha pessoas.`
  );
}

function applyStructuredPeopleFallback(textResponse = "", sources = []) {
  const crossRef = findPeopleCrossRefSource(sources);
  if (!crossRef?.text) return textResponse;

  const lines = extractPeopleLinesFromCrossRef(crossRef.text);
  if (lines.length === 0) return textResponse;

  const negatesPeople =
    /n[aã]o\s+(foram\s+)?encontrad|n[aã]o\s+h[aá]\s+pessoas|nenhum(a)?\s+nome\s+de\s+pessoa|sem\s+nomes?\s+de\s+pessoas/i.test(
      textResponse
    );
  if (!negatesPeople) return textResponse;
  ...
  return (
    `Pessoas citadas em documentos indexados que mencionam ${focus}:\n\n${body}\n\n` +
    `(Resposta montada a partir do cruzamento estruturado [CRUZAMENTO] dos documentos indexados.)`
  );
}
...
async function chatSync({
  ...
  const userPromptForLlm = injectStructuredPeoplePrompt(message, sources);
  ...
  const textResponse = applyStructuredPeopleFallback(rawTextResponse, sources);
...
async function streamChat({
  ...
  if (LLMConnector.streamingEnabled() !== true) {
    ...
    completeText = applyStructuredPeopleFallback(textResponse, sources);
    metrics = performanceMetrics;
    writeResponseChunk(response, {
      uuid,
      sources,
    ...
    completeText = applyStructuredPeopleFallback(
      await LLMConnector.handleStream(response, stream, { uuid }),
      sources
    );
